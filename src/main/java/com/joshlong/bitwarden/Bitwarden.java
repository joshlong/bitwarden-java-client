 ...
 * it is called, so pulling several values out of one vault entry means fetching it once and
 * passing the node to the {@code select} overloads:
 * {@snippet :
 ...
 * }
 * The {@code String itemId} overloads are conveniences for reading a single value; as their
 * default implementations show, each one fetches the entry again.
 */
interface Bitwarden {
