...
## CLOUD AMQP
# CLOUDAMQP_FILE="$HOME/mogul-secrets/cloudamqp"
# bw get item mogul-cloudamqp-production --raw > $CLOUDAMQP_FILE
# export RMQ_USERNAME="$(cat $CLOUDAMQP_FILE | jq -r '.fields[] | select(.name == "username") | .value'  )"
# export RMQ_PASSWORD="$(cat $CLOUDAMQP_FILE |  jq -r '.fields[] | select(.name == "password") | .value'  )"
# export RMQ_VIRTUAL_HOST="$(cat $CLOUDAMQP_FILE |  jq -r '.fields[] | select(.name == "username") | .value'  )"
# export RMQ_HOST="$(cat $CLOUDAMQP_FILE |  jq -r '.fields[] | select(.name == "hostname") | .value'  )"

###


export SPRING_RABBITMQ_HOST=$RMQ_HOST
...
## WORDPRESS
WP_FILE=$HOME/mogul-secrets/wordpress
bw get item mogul-wordpress-client--production  --raw > $WP_FILE
export WP_CLIENT_ID="$( cat "$WP_FILE" | jq -r '.fields[] | select(.name == "client-id") | .value')"
export WP_CLIENT_SECRET="$( cat "$WP_FILE"  | jq -r '.fields[] | select(.name == "client-secret") | .value')"
rm $WP_FILE

## ELASTICSEARCH
ELASTICSEARCH_FILE="$HOME/mogul-secrets/elasticsearch"
bw get item elasticsearch-development --raw > $ELASTICSEARCH_FILE
export ELASTICSEARCH_API_KEY="$(cat $ELASTICSEARCH_FILE | jq -r '.fields[] | select(.name == "api-key") | .value'  )"
export ELASTICSEARCH_API_HOST="$(cat $ELASTICSEARCH_FILE | jq -r '.fields[] | select(.name == "api-host") | .value'  )"
export ELASTICSEARCH_OTEL_HOST="$(cat $ELASTICSEARCH_FILE | jq -r '.fields[] | select(.name == "otel-host") | .value'  )"
export ELASTICSEARCH_OTEL_HEADER="$(cat $ELASTICSEARCH_FILE | jq -r '.fields[] | select(.name == "otel-header") | .value'  )"
...
## ABLY
export ABLY_KEY=$(bw get password mogul-ably-api-key-dev)

## OPENAI
export OPENAI_KEY=$( bw get password mogul-openai-key )
